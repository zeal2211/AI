...
#Adding the contents in the file to a list
for letter in content:
    original_board.append(letter)

#Swapping the board 
board = swap_color(original_board)
white_count = 8
black_count = 8 

#Find how many pieces have been placed
for i in board:
    if i == 'W':
        white_count -= 1
    if i == 'B':
...
def static_estimation_opening(board):
    ...
    return (1000*(num_white_pieces - num_black_pieces)) + total_neighbors_value - (len(generate_add(black_board)) * 10)
...
max_score = -math.inf
for board in boards:
    score = minimax(board, (int(depth_to_be_checked) - 1), max_score, math.inf, False, )
    if score > max_score:
        max_score = score
        best_move = board
        
original_move = swap_color(best_move) #Swapping the boards to get proper output
#Writing into the file given in the sys argv.
move = ''.join(original_move)
f = open(file_write,'w')
